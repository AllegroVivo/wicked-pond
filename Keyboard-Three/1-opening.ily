...
KeyboardThreeNumberOneRH = \relative c''' {
    ...
    \grace { s8 } \InstrumentChange \markup {
    ...
    r4 c2. ~ |
    c8 b4 b8 b2 |
    \InstrumentChange "Tremolo Strings" #-0.75
    a,,1:32-> |
    ...
    R1 * 2 | 
    \InstrumentChange "Voices 'Ahh'" #-0.6
    b,4( b' ~ b8 as fs4 ~ |
    ...
    R1 * 11 | 
    R1 \fermata | \ottava #1
    \InstrumentChange "Muted Stgs. Tremolo" #-0.75
    <e' gs b>1:32 ~ |
...
KeyboardThreeNumberOneLH = \relative c {
    ...
    R1 \fermata | \bar "||" \key b \major 
    R1 * 2 |
    \InstrumentChange "Symphonic Celli" #0
    b'4 b' ~ b8 as fs4 ~ |
    fs2 ~ fs8[ gs] \tuplet 3/2 { as b cs } |
    b2.( as4) | \time 6/4
    fs1. | \time 4/4
    R1 * 2 | \bar "||"
    \InstrumentChange "Bass Section Tremolo" #-0.75
    fs,1:32 ~ |
    fs:32 |
    ...
    R1 * 4 | \bar "||" \key d \major
    R1 * 2 |
    \InstrumentChange "Bass Section" #-0.75 \ottava #-1
    ds,,1 ~ |
    ds | \ottava #0
